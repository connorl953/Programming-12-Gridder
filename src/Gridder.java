...
public class Gridder extends javax.swing.JFrame 
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        for(int i = 0; i < 100; i++) {
            grid[rand.nextInt(100)][rand.nextInt(100)] = 1;
        }
        draw();
    }//GEN-LAST:event_jButton1ActionPerformed
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        int col = new Random().nextInt(100);
        for(int i = 0; i < 100; i++){
            grid[i][col] = 1;
        }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        textInfo.setText("There are " + count + " white squares");
        draw();
        ...
        for (int row = 0; row < 100; row++) {
            for (int col = 0; col < 100; col++) {
                ...
                if (grid[col][row] == 0) {
                    ...
                    if (col > 0 && row > 0 && grid[col - 1][row - 1] == 1) {
                        count++;
                    }
                    if (col > 0 && row < 99 && grid[col - 1][row + 1] == 1) {
                        count++;
                    }
                    if (col < 99 && row > 0 && grid[col + 1][row - 1] == 1) {
                        count++;
                    }
                    if (col < 99 && row < 99 && grid[col + 1][row + 1] == 1) {
                        count++;
                    }
                    if (count >= 2) {
                        temp[col][row] = 1;
